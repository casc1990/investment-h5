...
import { buildDividendTrade, rebuildPositionFromTrades, TRADE_TYPES } from '../shared/tradeEngine.js'
...
test('分红事件按持仓分红方式生成现金或红利再投流水', () => {
  const detail = { dividend_per_share: 0.004, ex_date: '2026-07-09' }
  const reinvest = buildDividendTrade({
    position: { quantity: 3697.65, dividend_method: '红利再投' },
    detail,
    confirmedNav: 1.25,
  })
  assert.deepEqual(reinvest, {
  ...
  const cash = buildDividendTrade({
    position: { quantity: 1000, dividend_method: '现金分红' },
    detail,
    confirmedNav: 1.25,
  })
  assert.equal(cash.trade_type, TRADE_TYPES.CASH_DIVIDEND)
  assert.equal(cash.amount, 4)
  assert.equal(cash.quantity, null)
})

test('红利再投缺少确认净值时拒绝入账', () => {
  assert.throws(() => buildDividendTrade({
    position: { quantity: 1000, dividend_method: '红利再投' },
    detail: { dividend_per_share: 0.004, ex_date: '2026-07-09' },
  }), /先同步基金净值/)
})
